...
LOGIN_OPERATIONS = ["authenticate", "connect"]
...
class RunOperation(Action):
    ...
    def _sanitize_operation_args(self, value_dict):
        """Converst all arguements to a camelCasedString
        :returns: dictionary of strings converted into camel case
        """
        op_args = {}
    ...
    def login(self, client, connection):
        ...
        if login_operation not in LOGIN_OPERATIONS:
            login_operation = conection_information['operation']
            for key, value in connection['kwargs_dict'].items():
                if key in login_operation["login_args"]:
                    operation_args[login_operation["login_args"][key]] = value
        else:
            login_operation = connection['operation']
            operation_args = context['kwargs_dict']

        op_args = self._sanitize_operation_args(operation_args)
        op_obj = client.service.__getitem__(login_operation)
    ...
    def _exec(self, context, client):
        ...
        if not context['session_id']:
            ...
            context['session_id'] = self.login(client, context['connection'])

        op_args = self._sanitize_operation_args(context['kwargs_dict'])

        op_obj = client.service.__getitem__(context['operation'])
        result = op_obj(sessionId=context['session_id'], **op_args)
        return result
